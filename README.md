...
A CSV export function, built into an earlier version of this script, will be restored in due course. This simply pushes all transaction lines out to STDOUT (for which reason the -V option is overriden).

This will be used to generated a "seed" data set for _OMsim_, a simple _Flask_ application to enable users to interact with each other, building upon a payment history generated by this script.

Some parts of this script will probably be reused in the _NESTS_ suite (https://nests.lrc.org.uk).
